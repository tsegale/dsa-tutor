...
const PSEUDOCODE: Record<string, string[]> = {
  ...
  'rb-delete': [
    ...
    '    Case 3/4: fix sibling\'s near child if needed, then recolour + rotate parent, done',
    '  root.colour = BLACK',
  ],
  'max-heap-insert': [
    'insert(value):',
    ...
    '  sift_up(last_index):',
    '    while parent exists and arr[i] > arr[parent]:',
  ...
  'max-heap-delete': [
    ...
    '  sift_down(0):',
    '    while a child is larger than arr[i]:',
    '      swap arr[i] with the larger child; i = that child',
  ],
  'min-heap-delete': [
    'delete():',
    '  move last element to the root, shrink the array',
  ...
  dfs: [
    'dfs(node):',
    '  mark node visited; record discovery time',
    '  for each neighbour (in lexicographic order):',
  ...
  dijkstra: [
    ...
    '    u = node with minimum dist, not yet finalised',
    '    finalise u',
    '    for each edge (u, v, weight):',
    '      if dist[u] + weight < dist[v]: dist[v] = dist[u] + weight (relax)',
  ],
  'bellman-ford': [
  ...
  kruskal: [
    ...
    '    else: skip - adding it would create a cycle',
  ],
  prim: [
    'prim(graph, start):',
    '  mst = {start}',
    '  while mst does not contain every node:',
    '    find the minimum-weight edge connecting mst to a new node',
    '    add that edge and node to the mst',
  ],
  'cycle-detection': [
    'has_cycle(graph):',
    '  colour every node WHITE',
    '  dfs(node): colour node GRAY',
    '    for each neighbour: if GRAY -> back-edge, cycle found',
    '    if WHITE: recurse',
    '  colour node BLACK when finished',
  ],
  'connected-components': [
    'connected_components(graph):',
    '  count = 0',
    '  for each unvisited node:',
    '    count += 1; BFS/DFS from it, marking everything reached with this component',
  ],
  'topological-sort': [
    ...
    '  colour node BLACK; prepend node to the output order',
  ],
  'grid-bfs': [
    'bfs(grid, start, end):',
    '  frontier = [start]; g[start] = 0',
    '  while frontier not empty:',
    '    current = dequeue from the front (FIFO)',
    '    if current == end: reconstruct path, done',
    '    for each open 4-directional neighbour:',
    '      relax neighbour, add to frontier',
  ],
  'grid-dfs': [
    'dfs(grid, start, end):',
    '  frontier = [start]; g[start] = 0',
    '  while frontier not empty:',
    '    current = pop from the top (LIFO)',
    '    if current == end: reconstruct path, done',
    '    for each open 4-directional neighbour:',
    '      relax neighbour, add to frontier',
  ],
  'grid-dijkstra': [
    ...
    '  frontier = [start]; g[start] = 0',
    '  while frontier not empty:',
    '    current = pop the cell with minimum g(n)',
    '    if current == end: reconstruct path, done',
    '    for each open 4-directional neighbour:',
    '      relax neighbour, add to frontier',
  ],
  'grid-astar': [
    ...
    '  frontier = [start]; g[start] = 0',
    '  while frontier not empty:',
    '    current = pop the cell with minimum f(n) = g(n) + h(n)',
    '    if current == end: reconstruct path, done',
    '    for each open 4-directional neighbour:',
    '      relax neighbour, add to frontier',
  ],
  'maze-generation': [
    ...
    '  every cell starts a wall',
    '  carve the starting cell; push it onto a stack',
    '  while stack not empty:',
    '    if the current cell has an unvisited neighbour two cells away:',
    '      carve the wall between them and the neighbour; push the neighbour',
    '    else: backtrack (pop the stack)',
  ],
  'maze-prim': [
    'generate_maze_prim(rows, cols):',
    '  every cell starts a wall',
    '  add the starting cell to the maze',
    '  frontier = walls adjacent to the maze',
    '  while frontier not empty:',
    '    pick a random frontier wall; carve it and its cell into the maze',
    '  add that cell’s new frontier walls',
  ],
  'maze-kruskal': [
    'generate_maze_kruskal(rows, cols):',
    '  every cell starts open, separated by walls',
    '  each cell begins as its own set (Union-Find)',
    '  walls = every wall between two adjacent cells, shuffled randomly',
    '  for each wall, in shuffled order:',
    '    if its two cells are in different sets: carve it, union the sets',
    '  else: skip - the cells are already connected',
  ],
  stack: [
