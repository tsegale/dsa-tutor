...
/** The exact text the Pseudocode tab renders for an algorithm, one line per
 * row. Sent with hint and feedback requests so the AI service quotes the
 * same variable names and array notation the student can see, rather than
 * its own separately maintained copy. Undefined when the tab has none. */
export function getPseudocodeText(algorithmSlug: string | undefined): string | undefined {
  const lines = algorithmSlug ? PSEUDOCODE[algorithmSlug] : undefined
