...
router.post('/', validate(S.interactions.create), requireOwnership({ in: 'body', key: 'sessionId' }), async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/session/:sessionId', validate(S.interactions.listForSession), requireOwnership({ in: 'params', key: 'sessionId' }), async (req: AuthRequest, res: Response) => {
  try {
